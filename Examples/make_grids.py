...
dat_row_header = '{0:13}{1:13}{2:13}{3:13}{4:13}{5:15}{6:13}'.format('Mass', 'Radius','Fe/Mg', \
                'Si/Mg', 'Bulk density', 'core_wt%', 'core_rad%')
np.savetxt(f_name, np.transpose([mass_grid,rad_grid, femg_grid, simg_grid, bulk_rho_grid, CMF_grid, CRF_grid]), \
            delimiter = '    ',  fmt = '%-10.4f', header = dat_row_header)
...
grid_size = int((len(FeMg))*(len(SiMg)))
...
for m in range(len(Mass)):
    ...
    for i in range(len(FeMg)):
        ...
        for j in range(len(SiMg)):
            ...
            #continue
            compositional_params = [wt_h2o,FeMg[i],SiMg[j],CaMg,AlMg,Fe_ox ,wt_Sic, \
                          wt_Oc , wt_Sc]
    ...
    np.savetxt(f_name, np.transpose([mass_grid,rad_grid, femg_grid, simg_grid, bulk_rho_grid, CMF_grid, CRF_grid]), \
                delimiter = '    ',  fmt = '%-10.4f', header = dat_row_header)
    #'{:^10}'.format('%f'
    #data_file.write('\n{0:>10.2}{1:>10.2}{2:>10.2}{3:>10.2}{4:>10.2}{5:>10.2}{6:>10.2}'.format(mass_grid, \
    #                        rad_grid, femg_grid, simg_grid, bulk_rho_grid, CMF_grid, CRF_grid))                
